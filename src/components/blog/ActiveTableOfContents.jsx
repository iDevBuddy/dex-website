        ...
        <>
            {variant !== 'mobile' && <aside className="hidden lg:block">
                <div className="sticky top-28 border-l border-border pl-5">
                    <p className="font-mono text-xs text-ghost-faint uppercase tracking-[0.2em] mb-4">Contents</p>
                                ...
                                <a
                                    ...
                                    className={[
                                        ...
                                        isActive
                                            ...
                                            : 'border-transparent text-ghost-dim hover:border-accent/50 hover:text-accent',
                                    ].join(' ')}
                                >
                                    {heading.text}
                                </a>
            ...
            {variant !== 'desktop' && <details className="lg:hidden mb-8 rounded-2xl border border-border bg-dark-deeper p-4">
                <summary className="cursor-pointer font-mono text-xs uppercase tracking-[0.2em] text-ghost-dim">Contents</summary>
                <nav className="mt-4 space-y-2">
                    {visibleHeadings.map((heading) => (
                        <a
                            ...
                            href={`#${heading.id}`}
                            onClick={(event) => handleClick(event, heading.id)}
                            className={`block text-sm ${activeId === heading.id ? 'text-accent' : 'text-ghost-dim'} ${heading.depth === 3 ? 'pl-4' : ''}`}
                        >
                            {heading.text}
                        </a>
                    ))}
