...
export default function VoiceChatbot() {
    ...
    try {
      ...
      const conv = await Conversation.startSession({
        ...
        onMessage: ({ message, source }) => {
          ...
          if (source === 'ai' && EMAIL_KEYWORDS.some(k =>
  ...
  const handleSubmit = async () => {
    ...
    const summary = messagesRef.current
      .map(m => `${m.role === 'user' ? 'Visitor' : 'Sarah'}: ${m.content}`)
      .join('\n')
    try {
  ...
  const labelText =
    ...
    status === 'connected' ? 'Listening...' : null
          ...
          >
            ...
            >
              {!submitted && (
                <button
                  onClick={closePopup}
                  className="absolute top-4 right-4 w-7 h-7 flex items-center justify-center rounded-full text-gray-500 hover:text-white"
              ...
              <AnimatePresence mode="wait">
                {submitted ? (
                  ...
                  <motion.div key="form" initial={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    ...
                    <p className="text-gray-400 text-sm mb-5">Drop your details and we will reach out shortly.</p>
                    <input type="text" value={nameValue}
                      onChange={e => { setNameValue(e.target.value); setNameError('') }}
                      onKeyDown={e => e.key === 'Enter' && handleSubmit()}
                      placeholder="Your name" autoFocus
                      className="w-full text-white text-sm px-4 py-3 rounded-xl outline-none placeholder-gray-600 mb-1"
                      style={{ background: '#111', border: nameError ? '1px solid rgba(239,68,68,0.5)' : '1px solid rgba(255,255,255,0.08)' }}
                    />
                    ...
                    <input type="email" value={emailValue}
                      ...
                      className="w-full text-white text-sm px-4 py-3 rounded-xl outline-none placeholder-gray-600 mb-1"
                      style={{ background: '#111', border: emailError ? '1px solid rgba(239,68,68,0.5)' : '1px solid rgba(255,255,255,0.08)' }}
                    />
                    ...
                    <input type="tel" value={phoneValue}
                      ...
                      className="w-full text-white text-sm px-4 py-3 rounded-xl outline-none placeholder-gray-600 mb-4"
                      style={{ background: '#111', border: '1px solid rgba(255,255,255,0.08)' }}
                    />
                    <button onClick={handleSubmit}
                      className="w-full py-3 rounded-xl text-white text-sm font-semibold tracking-wide hover:opacity-90"
